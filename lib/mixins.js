    ...
    constructor(options) {
        ...
        if (process.env.REDIS_PASS) {
            if (options.redis.password) {
                options.logger.log('warn/redis', 'Password is specified in environment variable and in config file - preferring environment variable');
            }
            options.redis.password = process.env.REDIS_PASS;
        }
