...
function _compileErrorCheckCondition(retryDefinition) {
    ...
    Object.keys(retryDefinition).forEach((catchCond) => {
        if (Array.isArray(retryDefinition[catchCond])) {
            const orCondition = retryDefinition[catchCond].
                map(option => createCondition(catchCond, option));
...
class Rule {
    ...
    _processMatchNot(matchNot) {
        ...
        if (Array.isArray(matchNot)) {
            const tests = matchNot.
                map((match) => (this._processMatch(match)).test).
                filter(func => !!func);
    ...
    test(message) {
        return this._options.
            findIndex(option => option.match(message) && !option.match_not(message));
    }
