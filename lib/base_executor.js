...
class BaseExecutor {
    ...
    async subscribe() {
        ...
        this.consumer = await this.kafkaFactory.createConsumer(
        ...
        timers.setTimeout(this.kafkaFactory.startup_delay)
            .then(() => this._consume());
    }
    ...
    async _consume() {
        ...
        try {
            const messages = await this.consumer.consumeAsync(this.consumerBatchSize);
            if (!messages.length) {
                // No new messages, delay a bit and try again.
                return timers.setTimeout(100);
            }
    ...
    _notifyFinished(finishedMsg) {
        ...
        if (!this._commitTimeout) {
            this._commitTimeout = setTimeout(async () => {
                ...
                await Promise.all(toCommit.map(async (message) => {
                    try {
                        await this.consumer.commitMessageAsync(message);
                    } catch (e) {
    ...
    async _exec(origEvent, handler, statDelayStartTime, retryEvent) {
        ...
        if (this.rule.shouldAbandon(origEvent)) {
            ...
            this._hyper.metrics.increment(`${ this.statName(origEvent) }_abandoned`);
            return { status: 200 };
        }
