...
class RetryExecutor extends BaseExecutor {
    get subscribeTopics() {
        const newRetryTopics = this.rule.topics.map(topic =>
            `${this.kafkaFactory.consumeDC}.${this.retryTopicName(topic)}`);
        // TODO: no need to subscribe to all of them as soon is we transition
        return newRetryTopics.concat(this.rule.topics.map(topic =>
            `${this.kafkaFactory.consumeDC}.change-prop.retry.${topic}`));
    }
    ...
    getHandler(message) {
        ...
        if (message.emitter_id !== this.emitterId()
                ...
                && message.emitter_id !== this._oldEmitterId()) {
