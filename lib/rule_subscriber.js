...
class Subscriber {
    ...
    async _createSubscription(hyper, ruleName, ruleSpec) {
        if (Rule.isBasicRule(ruleSpec)) {
            return new BasicSubscription(this._options,
                this._kafkaFactory, hyper, ruleName, ruleSpec);
        }

        if (!this._metadataWatch) {
            this._metadataWatch = await this._kafkaFactory.createMetadataWatch(
                'metadata_refresher'
            );
        }

        return new RegexTopicSubscription(this._options, this._kafkaFactory,
     ...
     * @return {Promise}
     */
    async subscribe(hyper, ruleName, ruleSpec) {
        const subscription = await this._createSubscription(hyper, ruleName, ruleSpec);
